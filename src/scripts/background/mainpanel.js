...
var Record = (function RecordClosure() {
  ...
  Record.prototype = {
    ...
    addEvent: function _addEvent(eventRequest, portName) {
      ...
      if (this.recordState == RecordState.RECORDING) {
        ...
        if (this.recentEvents.length > 0){
			...
			var currEventType = this.eventCategory(eventRecord);
			this.lastEventType = currEventType;
	...
	addEventsToSend: function _addEventsToSend(interpretedEvent){
		...
		else if (interpretedEvent.type == "paste"){
			var e = interpretedEvent.events[interpretedEvent.events.length-1];
			e.target = interpretedEvent.target;
			e.type = "paste";
			e.msg.type = "paste";
			e.msg.startWord = interpretedEvent.startWord;
			e.msg.endWord = interpretedEvent.endWord;
			this.eventsToSend.push(e);
		}
		else if (interpretedEvent.type == "copy"){
			var e = interpretedEvent.events[0];
			e.msg.type = "copy";
			e.type = "copy";
			e.msg.target = interpretedEvent.target;
			e.target = interpretedEvent.target;
    ...
    eventCategory: function _eventCategory(eventMsg){
    	...
    	if (["textInput","keypress","keyup","keydown","input"].indexOf(eventType) > -1){
			if (this.lastEventType == "paste"){
				//recall that a textInput gets sent off after a paste
				return "paste";
			}
    		return "type";
    	}
		...
		*/
    	...
    	if (targetNodeName == "select"){
			...
			if (this.lastRecordedCopy != null){
				...
				var segments = potentialCopyText.split(actualPastedText);
				var wordsBefore = segments[0].split(" ");
				var wordsIn = actualPastedText.split(" ");
				interpretedEvent["startWord"] = wordsBefore.length-1;
				interpretedEvent["endWord"] = wordsBefore.length+wordsIn.length-1;
			}
			interpretedEvent['display'] = "You pasted: "+actualPastedText;
		}
    	else{
    ...
    setClipboard: function _setClipboard(str){
		this.clipboard = str;
	},
    getClipboard: function _getClipboard(){
...
var Replay = (function ReplayClosure() {
  ...
  Replay.prototype = {
    ...
    prepareMsg: function _prepareMsg(msg){
		if (msg.type == "paste"){
			msg.text = this.record.getClipboard();
			console.log("got clipboard: "+msg.text);
		}
		return msg;
	},
    ...
    guts: function _guts() {
      ...
      if (replayState == ReplayState.WAIT_ACK) {
        ...
        if (!("checkedWait" in e)) {
          ...
          // send message
          try {
			//we only really need to prepare the message once
...
var handleMessage = function(port, request) {
  if (request.type == 'event') {
    ...
    record.setClipboard(request.value);
    bgLog.log('got clipboard: '+request.value);
